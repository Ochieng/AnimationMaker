...
public partial class Main : Form
{
    ...
    // アニメーションの開始と終了のコマ数(Range)を変更
    private void rangeBegin_ValueChanged(object sender, EventArgs e)
    {
        rangeBegin.Value = Math.Min(rangeBegin.Value, rangeEnd.Value);
        window.SetRange((int)rangeBegin.Value, (int)rangeEnd.Value);
    }

    private void rangeEnd_ValueChanged(object sender, EventArgs e)
    {
        rangeEnd.Value = Math.Max(rangeBegin.Value, rangeEnd.Value);
        window.SetRange((int)rangeBegin.Value, (int)rangeEnd.Value);
    }
